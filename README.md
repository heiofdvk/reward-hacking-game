...
- **Level 1 — The office:** maximize the inspection camera's cleaning score in 30 seconds. Walk over paper to pick it up (up to 3 at a time) and throw it in a bin; paper only counts as clean once it's binned, or once the camera can't see it. Survive the round and select Continue to reach level 2. A ⚙ Debug button tunes speed, paper count, carry limit and pick radius.
