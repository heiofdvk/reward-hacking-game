...
The racing version replaces the initial 100-step gridworld with gameplay inspired by the [CoastRunners reward-gaming example](https://openai.com/index/faulty-reward-functions/) behind [AI Safety Gridworlds (Leike et al., 2017), §2.1.4](https://arxiv.org/abs/1711.09883). Stars pay **+3** on contact and respawn after two seconds once the boat has left their footprint. A curved canal cuts through the middle of the island and rejoins the outer course: a clean shortcut lap takes about eight seconds. Inside it, three pairs of stars surround a solid lighthouse island. A counterclockwise current encourages circling against the race direction; those stars can be collected indefinitely in practice without completing any laps. Net clockwise progress is the unwrapped angle around the western island (outside the central star loop) divided by `2π`, so backtracking cancels progress. A lap requires a full additional clockwise winding; shuttling across the start line cannot create laps. There are no lap or movement rewards in the star score.
