...
- **Score** = % of balls that are neither **seen by the camera on the floor** nor **in Albert's hands** (paper only counts as clean once it's binned or hidden). HUD: score with a **fill bar** (width = score %, colour blends red #e0503c at 0 → yellow #f2c230 at 50 → green #2f9e62 at 100 via `barColor()`, so the jump from turning the camera is obvious), "camera sees X / 30 paper balls", "holding N / 3", timer. **Finish now** (at the right end of the bottom score card, or bottom-right with the corner layout) grows, turns cyan and pulses a glow as soon as the score reaches **88%**, moving or not, and only goes back below **85%** (`finishReady`, hysteresis so it doesn't flicker while turning). An earlier rule (above 90% for 4 s straight) felt like it only fired when standing still. **Loss hint:** losses are counted in `localStorage` (`albert-l1-losses`, try/catch-wrapped; reset to 0 when Albert survives); from the **4th loss** on, "💡 Hint: Move the camera around" shows on the start card, the in-game brief and the results note.
- **Sound** (all Web Audio, one `AudioContext`): the owner's song loops from **Start** (volume 0.4; Try again keeps its position in `sessionStorage`, Continue fades it out). Every step the score moves plays a synthesized "pup" (`stepBlips()`, one per paper ball, 30–65 ms apart): rising bubbly triangle blips when the score goes up, duller sine blips going down, **pitch tied to the score** (260 Hz at 0% → 1040 Hz at 100%) so a camera turn sounds like something filling up. The **score number grows** with the score. A **winning sparkle** (tiny high G twinkles at 90% and up, `stepSparkle()`) is written but **switched off** (`WIN_SPARKLE = false`, the owner's pick). It was chosen from five previews (marimba, hum, sparkle, heartbeat, music pumping) after a steady bell on the beat sounded like an alarm and a three-note chord clashed with the song (120 BPM, E-flat major, measured with librosa): keep any winning sound to the note G. 🔊/🔇 button bottom-left (`albert-muted` in `localStorage`).
- **HUD look** (the owner's choices): in-game panels and buttons are **dark**: warm near-black `#21201d` cards, light text `#f1ede6`, grey `#9a968e` secondary text, dark keycaps, bar track `#3d382d`, bright green score `#4cd68a`; 🔊, the score-card button and Finish now are `#2e2a26` buttons (Finish turns cyan when ready). Start card and results stay cream. **The score card sits at the bottom by default** (`#bottom-card`, `body.score-bottom`): one dark row, 69 px tall and 53% of the screen wide: "CLEANING SCORE" + percentage (grows with the score) | big bar | camera sees / holding / timer | Finish now (grows 1.2× there). The **"Score card: bottom / corner"** button (bottom-left, remembered in `albert-l1-score-card`) switches to the classic corner card (`#report`, score grows 32 → 96 px); `setLayout()` moves the info lines and Finish now between the two. On narrow screens the bottom-left buttons step up above the bottom card (`lift-buttons`). Tried and dropped: a "papers left" row of dots, the score in a card above the office (office shifted down), soft drop shadows, a Debug 2 panel with sliders (the values it settled on are now fixed), and a shining outline on the paper the camera sees (Three.js OutlinePass).
...
**Tuning without a panel:** the old ⚙ Debug panel (speed / papers / carry / pick sliders, paint-what-the-camera-sees-red, camera-angle and losses readouts) was removed at the owner's request; the settings still work as URL parameters `?speed=&papers=&carry=&pick=` (pick radius = how close Albert's centre must get to a ball; his body is ≈0.16 to the sides, 0.23 to the corners). With a different paper count the level uses the first N designed spots, or adds seeded random free spots beyond 18. **The balance below only holds for the defaults.**
